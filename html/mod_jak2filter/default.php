    ...
    <ul id="jak2filter<?php echo $module->id; ?>" class="ja-k2filter-wrap <?php echo $ja_stylesheet; ?>">
            ...
            <?php if ($filter_sort != false) : ?>
                <?php foreach ($filter_sort as $kfs => $fs): ?>
                    <?php if (in_array($kfs, $order_display)): ?>
                        <li class="fssorts" data-of="<?php echo(ltrim($fs, 'r')); ?>">
                            <div class="title">
                            <h6 class="fsname"><?php echo(ltrim($fs, 'r')); ?></h6>
                            </div>
                            <div class="group-controls">
                            <?php if (!in_array($kfs, array('zelevance', 'best', 'modified', 'publishUp', 'zdate', 'featured'))) : ?>
        ...
        <li <?php echo $style; ?> class="last-item">
            <?php if ($params->get('auto_filter', 1) == 0): ?>
                <input class="btn btn-primary" type="submit" name="btnSubmit" value="<?php echo JText::_('JAK2SEARCH'); ?>"/>
            <?php endif; ?>
            <?php if ($params->get('enable_reset_button', 1) == 1): ?>
                <input class="btn btn-default" type="button" name="btnReset" value="<?php echo JText::_('RESET'); ?>" onclick="jaK2Reset('<?php echo $module->id; ?>', '<?php echo $formid; ?>', true);resetChosen(<?php echo $module->id;?>);"/>
            <?php endif; ?>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#jak2filter<?php echo $module->id;?>").sortable({
            ...
            <?php if($auto_filter || $ajax_filter): ?>
        ...
        jQuery('#jak2filter<?php echo $module->id;?> #ordering').change(function () {
            if (!jQuery('#jak2filter<?php echo $module->id;?>').find('li[data-of=' + jQuery(this).val() + ']').length) {
            	strout = '';
        ...
        jQuery("select").chosen().change(function(){
        	document.id('<?php echo $formid; ?>').fireEvent('submit');
        });
        <?php endif; ?>
    });
